...
int main() {
  ...
  srand(time(NULL));
  lst_push_back(one, 5);
  for(i = 1; i <= 10; ++i) { lst_push_back(medium, i); }
  ...
  */
  ...
  for (i=1; i<=5; ++i) { lst_push_back(evenList, i*2); }
  ...
  FREE(evenList);
  evenList = lst_create();
  for(i=1; i<=5; ++i) { lst_push_back(evenList, i*2); } // Reset value for later tests.
  ...
  oddList = lst_create();
  for(i=0; i<5; ++i) { lst_push_back(oddList, i*2+1); }
  ...
  for (i=1; i<=5; ++i) { lst_push_back(evenList, i*2); lst_push_back(dupsOfevenList, i*4); }
  ...
  evenList = lst_create();
  oddList = lst_create();
  for(i=1; i<=5; i++) { lst_push_back(evenList, i*2); lst_push_back(oddList, i*2-1); }  // 2 4 6 8 10
