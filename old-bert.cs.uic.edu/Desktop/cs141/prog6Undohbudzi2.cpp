...
void displayIdentifyingInformation()
{
    ...
    printf("TA: Michael Irizzary, Tu 5pm \n");
                      ...
                      int &numberOfWords)      // number of words stored
{
    ...
    int dictionarySize = 1;	  // User input sets this, which in turn determines which dictionary to open
    ...
    pInputFile = fopen("smallDictionary.txt", "r");   // Returns null if not found.  Mode is "r" for read
...
//--------------------------------------------------------------------------------------
// Uaase binary search to look up the search word in the dictionary array, returning index
...
int main() {
    ...
    // creating list pointer and intialied to NULL
    Node *pHead = NULL;
    ...
    // creates first Node in a linked list
    insertNode(pHead, board, moveNumber);
    
    // display the board and list number
    displayBoard( board);
    printf("\n");
	printf("   List: ")	;
    theNode(pHead);
    ...
    while( ! done( board, dictionary, numberOfWords)) {
        ...
        if( rowOrColumn == 'x') {
            ...
            printf("You chose 'u' to undo a move. \n");
            removeNode( pHead, board, moveNumber);
            displayBoard( board);
			printf("\n");
			printf("   List: ")	;
	        theNode(pHead);
            ...
            printf("You chose 'o' for original board to be shown \n");
