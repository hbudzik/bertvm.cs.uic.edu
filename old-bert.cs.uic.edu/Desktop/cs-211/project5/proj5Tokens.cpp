...
// defining all the Darray class methods here
	...
	//push value onto the stack
		void Darray::push(int val)
		{
			//check if stack is full, allocate more memore if true
			if (inUse == size)
			{
				int *temp;
				temp = (int *)malloc(sizeof(int) * (size + 2));		//increases mem allocation by 2
			...
			//push on the stack
			darr[inUse] = val;		//pushes onto the top of the stack
			inUse++;
	...
	// prints the info of the stack
		void Darray::printStack()
		{
			printf("value stack --> ");
			int temp = inUse;
